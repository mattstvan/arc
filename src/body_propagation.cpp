...
@throws exceptions::ArcException if ephemeris file is not found or propagation fails
*/
ICRF BodyPropagationHandler::get_state(int id, DateTime& epoch) {
  ...
  if (id != 10) {
    try {
      ...
      throw ArcException("BodyPropagator::get_state exception: Propagation of CelestialBody failed");
    }
  }
  else {
